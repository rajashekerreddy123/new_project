(function(jQuery) {
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		if(jQuery('div#sticky-container').hasClass("sticky-container")){}
			else{
	...
	jQuery.fn.stickyNotes.createNote = function() {
		...
		var note_id = parseInt(jQuery("#last_note_id").val()) + 1;
		jQuery("#last_note_id").val(note_id)
		...
		var _div_edit = 	$(document.createElement('div'))
							.addClass('jSticky-edit')
							.click(function(){jQuery.fn.stickyNotes.editNote("#p-note-" + note_id);});
		...
		$.ajax({
        	url: '/accordions/create_note/1',
        	data: {'description': '', 'note_id': note_id}
        })
	...
	jQuery.fn.stickyNotes.removeCurrentlyEditedNote = function(note_id) {
		$.ajax({
        	url: '/accordions/update_note/1',
        	data: {'description': jQuery.fn.stickyNotes.getNote(note_id).text, 'note_id': note_id}
        })
	...
	var _div_wrap1 	= 	$(document.createElement('div'))
	jQuery.fn.stickyNotes.renderNote = function(note) {
		...
		var _div_delete =""
		var _div_edit
		if(note.permissions == jQuery('#user_id').val() ){
		 _div_delete = 	$(document.createElement('div'))
							.addClass('jSticky-delete')
							.click(function(){jQuery.fn.stickyNotes.deleteNote(this);});
		 _div_edit = 	$(document.createElement('div'))
							.addClass('jSticky-edit')
							.click(function(){jQuery.fn.stickyNotes.editNote("#p-note-" + note.id);});
		}
		var _div_wrap 	= 	$(document.createElement('div'))
							.css({'position':'relative','top':note.pos_y,'float': 'right',"width":note.width,"height":note.height})
		...
		_div_wrap1.append(_div_wrap)
		$('#sticky-container').append(_div_wrap1);
